...
uint32_t* g_bg = 0;
uint32_t* g_bg_next = 0;
float g_bg_blend = 0.0f;
int g_bg_blending = 0;
...
static void font_cache_clear(void) {
    for (int i = 0; i < FONT_CACHE_CHARS; ++i) {
        if (g_font_cache[i].bitmap) {
            free(g_font_cache[i].bitmap);
            g_font_cache[i].bitmap = 0;
        }
        g_font_cache[i].w = 0;
        g_font_cache[i].h = 0;
    }
...
int g_desktop_dirty = 1;
uint64_t g_last_key_tick = 0;
char g_last_key_char = 0;
...
desk_term_route_t g_term_routes[DESK_TERM_ROUTE_MAX];
desk_term_state_t g_term_states[DESK_MAX_WINDOWS];
desk_term_state_t* g_term_exec_state = 0;
uint8_t g_key_last[128];
uint8_t g_term_passthrough = 0;
...
char g_wallpaper_pref[320] = "gradient";
int g_wallpaper_custom = 0;
int g_wallpaper_builtin_enabled = 0;
...
#endif
uint8_t* g_wallpaper_file = 0;
size_t g_wallpaper_file_cap = 0;
...
long desktop_launch_by_basename_tid(const char* base) {
    char candidate[256];
    char app_dir[256];
