...
static __attribute__((noreturn)) void ntux_fatal_loop(const char* msg) {
    if (msg && msg[0]) {
        puts("I_Error:");
        puts(msg);
    } else {
        puts("I_Error: (no message)");
    }
    for (;;) {
        sys_wait_ticks(100);
    }
...
void I_Error (char *error, ...)
{
    ...
    // NTux: print and halt so we can see the error, avoid NULL callbacks.
    ntux_fatal_loop(msgbuf);
    ...
    // abort();
    // Unreachable on NTux due to early loop above.
}
