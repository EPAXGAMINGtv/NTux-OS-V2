...
static inline uint64_t v2p(uint64_t virt) {
    ...
    const uint64_t phys_mask = 0x0000FFFFFFFFF000ULL;
    cr3 &= ~0xFFF;
    ...
    uint64_t* pdpt = (uint64_t*)(hhdm + (pml4e & phys_mask));
    uint64_t pdpte = pdpt[(virt >> 30) & 0x1FF];
    if (!(pdpte & 1)) return 0;
    if (pdpte & (1 << 7))
        return (pdpte & phys_mask) | (virt & 0x3FFFFFFF);

    uint64_t* pd = (uint64_t*)(hhdm + (pdpte & phys_mask));
    uint64_t pde = pd[(virt >> 21) & 0x1FF];
    if (!(pde & 1)) return 0;
    if (pde & (1 << 7))
        return (pde & phys_mask) | (virt & 0x1FFFFF);

    uint64_t* pt = (uint64_t*)(hhdm + (pde & phys_mask));
