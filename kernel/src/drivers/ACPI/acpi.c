...
static volatile struct limine_rsdp_request acpi_rsdp_request = {
...
void acpi_shutdown(void) {
    ...
    if (acpi_fadt) {
        uint16_t pm1a = (uint16_t)acpi_fadt->pm1a_cnt_blk;
        uint16_t pm1b = (uint16_t)acpi_fadt->pm1b_cnt_blk;
        // Try SLP_TYP 7 (QEMU PIIX4) and 5 (common ACPI) for S5
        static const uint16_t s5_types[] = { 7, 5, 0 };
        for (size_t i = 0; i < sizeof(s5_types) / sizeof(s5_types[0]); i++) {
            uint16_t val = (s5_types[i] << 10) | SLP_EN;
            outw(pm1a, val);
...
int acpi_init(void){
    ...
    // acpi_i2c_enumerate();  // Disabled: DSDT walk causes page fault on this platform
    // i2c_lpss_init();
